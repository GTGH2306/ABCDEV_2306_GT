...
public class Aquarium {
	...
	public void tour(){
		...
		for (int i = 0; i < sizePoissonsTour; i++) { //Remet les poissons � l'�tat "ne c'est pas reproduit" ce tour
			this.poissons.get(i).setReproduced(false);
		}
		System.out.println("----TOUR " + this.tour + "----");
		for (int i = 0; i < sizeAlguesTour; i++) { //Fais le comportement qu'on les algues � chaque tours
			if (this.algues.get(i).isAlive()) {
				this.algues.get(i).comportement();
			}
		}
		for (int i = 0; i < sizePoissonsTour; i++) { //Fais le comportement qu'on les poissons � chaque tours
